...
function DockTile({ item, size }: { item: DockItem; size: number }) {
  ...
  return (
    <span
      className="flex items-center justify-center border border-glass-edge text-foreground/90"
      ...
      style={{ ...TILE_STYLE, width: size, height: size, borderRadius: 4 }}
    >
      {customIcon ? (
        <Image
          src={customIcon}
...
export function Dock({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 flex justify-center border-t border-glass-edge bg-background/42 px-2 pb-2 pt-2 backdrop-blur-md sm:pb-3">
      <div
        ref={containerRef}
        onMouseMove={onMouseMove}
        onMouseLeave={onMouseLeave}
        className="pointer-events-auto flex max-w-full items-center gap-1.5 overflow-x-auto px-3 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        style={{
        ...
        {items.map((item, index) => {
          ...
          const control =
            item.action === 'link' && item.url ? (
              <a
                ...
                className="shrink-0 cursor-pointer rounded-sm focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
              >
                {tile}
              </a>
            ) : (
              <button
                ...
                className={cn(
                  'shrink-0 rounded-sm focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
          ...
          return (
            <Fragment key={item.id ?? item.label}>
              {item.dividerBefore ? (
                <span
                  className="mx-1 h-8 w-px shrink-0 self-center bg-border"
                  aria-hidden="true"
                />
              ) : null}
              <Tooltip>
                ...
                <TooltipContent
                  side="top"
                  sideOffset={12}
                  className="hidden border border-glass-edge bg-card text-foreground shadow-lg sm:block [&_[data-slot=tooltip-arrow]]:bg-card"
                >
                  {item.label}
                </TooltipContent>
              </Tooltip>
