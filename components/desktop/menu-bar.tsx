...
function Clock({ locale }: { locale: string }) {
  ...
  return (
    <span className="min-w-[11rem] text-right font-data text-[0.68rem] text-foreground/68 capitalize">
...
export function MenuBar({
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-glass-edge bg-background/40 backdrop-blur-md transition-all duration-500">
      <div className="px-4 md:px-6">
        <div className="flex h-12 items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="sr-only">{site.ownerName}</span>
            <LivingTag compact className="text-primary" />
            <span className="hidden font-data text-[0.62rem] text-foreground/55 sm:inline">
              living archive
            </span>
              ...
              <nav className="ml-2 hidden items-center gap-4 md:flex">
                {nav.map((link) => (
                  <button
                    ...
                    className={cn(
                      'cursor-pointer rounded-sm font-data text-[0.68rem] transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
                      isActive(link) ? 'text-foreground' : 'text-foreground/62'
                    )}
                  >
                    {link.label}
                  </button>
          ...
          <div className="md:hidden">
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              <SheetTrigger
                ...
                className="inline-flex size-9 cursor-pointer items-center justify-center rounded-md text-foreground transition-colors hover:bg-foreground/10 focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
              >
